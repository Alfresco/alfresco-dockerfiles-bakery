...
do_fetch_mvn() {
    for i in $(yq e ".artifacts | keys | .[]" "$1"); do
    ARTIFACT_REPO=$(yq e ".artifacts[\"$i\"].repository" "$1")
    ARTIFACT_NAME=$(yq e ".artifacts[\"$i\"].name" "$1")
    ARTIFACT_VERSION=$(yq e ".artifacts[\"$i\"].version" "$1")
    ARTIFACT_EXT=$(yq e ".artifacts[\"$i\"].classifier" "$1")
    ARTIFACT_GROUP=$(yq e ".artifacts[\"$i\"].group" "$1")
    ARTIFACT_PATH=$(yq e ".artifacts[\"$i\"].path" "$1")
...
TARGETS=$(find "${REPO_ROOT}" -regex "${REPO_ROOT}/${1:+$1/}.*" -name "artifacts-${ACS_VERSION}.yaml" -mindepth 2 -print)
